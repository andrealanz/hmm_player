...
# In[23]:
names = os.listdir("in_CSV");
for i in names:
    csv_path = 'in_CSV/' + i
    output_path = 'out_CSV/' + i
    np.random.seed(77)
    try:
        time, old2, new2, z2, pi2, phi2, Tmat2 = hmm_compose(csv_path, output_path, 256, 'first_order', 2, 10E-7, 1, metrics_calc = False, case_study = True)
    except:
        print("Error in file: ", csv_path)
        pass
    
print("Done")
# In[24]:
names = os.listdir("out_CSV")
os.chdir("out_CSV/")
for i in names:
    if i != 'Csvmidi.exe':
        file_name = i.partition("_")[0] + '.mid'
        cmd =  'Csvmidi.exe ' + i + ' ' + file_name 
        if os.system(cmd) == 1:
            print("Error in csvmidi")
